...
- Host: macOS 26.6.2 build 25G83 arm64, .NET SDK 10.0.400, PowerShell 7.6.5.
- Exact clean local product head `0c9328a`: Pester 113 passed with 3 platform
  skips; server 1,360/1,360 with two known analyzer warnings; SIEM 357/357
  clean.
...
- A fresh `osx-arm64` layout at provisional version
  `0.0.0-stream-clean+0c9328a` passed layout validation, packaged staged
  activation, and the 30-check direct product proof with clean-source
  enforcement, including information/verbose response and recovery retention.
- A separate isolated public source install under
  `/Users/michael/.ptk-stream-clean.*` passed both package and installed
  handshakes, followed by the 32-check direct product proof including opt-in
  uninstall. Progress stayed excluded. Both explicit throwaway roots were
  removed after proof.
...
- Source installer built commit `38058697683a03a85db18139820bd1159475d89a` as `0.2.0-dev.g3805869`, passed staged and activated package handshakes, and installed `/Users/michael/.ptk/bin/PtkMcpServer`. It registered that exact path user-wide for Claude, Codex, Grok, agy, and Kimi. Installed SHA-256 values: server `2501a83d0d23b28744d273b3c6857b5d06983a4e5d7fd841f23720878367bc24`; server DLL `dbf9a61d19e8e7d2ccf010bda32264c3c34f90028f548edb3077042865eed1bd`; Unix broker `6595946d6e48162c7439e60bb77cddb288ce4c9e13311082fbb91c3754e2e980`.
- The exact installed server passed the full registration handshake with explicit distinct `HTTP_PROXY`/`http_proxy` and `HTTPS_PROXY`/`https_proxy` values. A fresh `claude mcp get ptk` under that same duplicate-case environment reported user-scope stdio status `Connected`; `codex mcp get ptk` reported the installed command enabled. Already-open clients with the killed old transport still require a fresh session.
- Before handoff, the owner reported PTK uninstalled and moved continued development to Windows Server. A subsequent read-only Mac check still found `/Users/michael/.ptk/bin/PtkMcpServer`, `/Users/michael/.ptk/VERSION`, and registrations pointing to that server in Claude, Codex, Grok, agy, and Kimi configuration. Therefore the Mac uninstall is incomplete or referred to a different surface; no cleanup was attempted during handoff.
...
## `nagatha.local` current-source SIEM capture and prospective-add repair (2026-08-15)

- The first isolated proof used source `e7a95740f16f9cc9cfd2961312e543cb80467f6a`, a fresh `0.3.0-s5` receiver, and a separately built PTK producer. Adding the receiver live to an already-running producer made sequence 2 the first obligated record. The receiver quarantined sequence 2 as `chain_gap`, continued the same batch, stored sequence 3 post-gap, and thereafter rejected every isolated sequence-2 retry as `chain_position`. Producer status reached `export.evidence_refused`, two pending core records, two pending evidence records, seven consecutive failures, and 14 quarantine attempts. This disproved the prior packaged Doctor workflow as a sufficient prospective-add/delivery-order acceptance test.
- Implementation `e0aea02b9f33498d0b4b6a63017778633ef640a1` stops JSON batch processing after a permanent refusal of a validated record so producer isolation retries that chain position before later validated records can overtake it. The new regression failed before the guard with sequence 3 already stored, then passed after restoration. Focused gap/JSON ingest passed 19/19; full `dotnet test siem/PtkSiem.slnx --no-restore --nologo -v minimal` passed 357/357; `git diff --check` was clean.
- Exact-commit packages were rebuilt under protected `~/.ptk-siem-capture-demo-fixed`: receiver archive SHA-256 `d3c63bc0d614fd6ac0fb1cbca091bc6caf4192a05bc4b11d13259a200e30de6f`; producer executable SHA-256 `633dfbef35d950ca2411421ce25269de1cbe652014a8d1dc67cd7b1a18cd8ed7`. A live destination add required no restart. Attributed call `01a00577-eca6-701d-874a-6b7d6eb8f931` stored `call.accepted`, `call.completed`, `evidence.submitted_command`, and `evidence.caller_response`; agent/model were `codex` / `openai` / `gpt-5` with `client_asserted` strength. Producer delivery ended healthy with zero pending event/evidence records, zero refused/missing records, zero consecutive failures, and six delivered records. Receiver health retained one open prospective-prefix gap and one opening `chain_gap` quarantine attempt, which is visible and does not block delivery. The protected, token-free operator capture report is `~/.ptk-siem-capture-demo-fixed/captured-activity.json`, SHA-256 `b4bc1c806a71ff952ddbffd33b157326cd2443c72fe745061ddd9e905b631b91`; receiver and producer were left running for owner inspection.
- The same live activity exposes two independent capture defects. `requested_cwd` is correct, but `effective_cwd` is null with `not_dispatched` and repository context absent despite successful execution. Its caller response advertises `ptk_output` recovery; direct `ptk_output status` proved the inner artifact available, complete, 176,000 bytes, and `powershell_objects`, but the activity evidence manifest contains only `submitted_command` and `caller_response`, not `captured_output`. Both remain uncorrected.
