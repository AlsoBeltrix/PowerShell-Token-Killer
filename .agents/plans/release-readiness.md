...
**Status:** UNPUBLISHED RC.2 BUILT; CHECKSUM CORRECTION VERIFIED — on 2026-09-04
...
| 6. Validate the exact candidate | **PARTIAL — packaging verified** | Source `6ab7040` passed six-job CI `33935459185` and all six release jobs in `33935468032`. The owner explicitly approved a checksum-only correction on draft `383097364`; fresh GNU/Perl consumer checks, twelve digests, eleven file hashes, ten identities, and unchanged Mac archive proof pass. `rr-8` is closed without changing binaries/version/source. Remaining native/downloaded and public-launch gates are in `.agents/release-candidate-0.3.0-rc.2.md`. |
