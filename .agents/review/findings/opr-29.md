...
The repair boundary extends through `UnixWorkerProcessLauncher.LaunchAsync`: it re-materializes `command.Environment` with `StringComparer.OrdinalIgnoreCase` before adding the two bootstrap descriptors. After the command snapshot becomes Unix-ordinal, this copy would still throw on case-distinct source keys; if a case-distinct lowercase bootstrap-like name survives, the uppercase descriptor assignment would overwrite it. Fixing only `WorkerLaunchCommand` or `CaptureEnvironment` therefore cannot restore Unix environment identity through spawn.
...
Apply platform-correct environment identity in both `WorkerLaunchCommand` and the Unix launcher's environment copy. The Unix spawn-boundary guard must also preserve a lowercase bootstrap-like variable separately from the uppercase injected descriptor variables; Windows keeps case-insensitive duplicate rejection and reserved-name custody.
