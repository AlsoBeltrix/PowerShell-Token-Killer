...
**Status**: **FIXED 2026-08-06** (`11eafee`, `c40404a`, `18d76e8`). Owner ruled for structured content over escaping the text channel. PTK's own decisions now travel in the protocol's `structuredContent` (`disposition`, `executed`, `safe_to_resubmit`, `detail`) and `isError`, neither of which worker output can reach; the response text is unchanged byte-for-byte, so no user output is altered and nothing that parses it breaks. Proved on the wire against a real built server: the forged status line and handle survive in the text while the verdict reads `completed`/`executed=true`, and a genuine refusal reads `not_started`/`isError=true`. Two follow-on defects were found and fixed in the same effort — a refusal raised *after* the work began (`descendants_unknown`) claiming `safe_to_resubmit=true`, and a worker-reported non-start (`WorkerResultStatus.Refused`, which returns normally rather than throwing) reported as `executed=true`. This also closed the deferred refusal→`isError` mapping: `SupervisorCallFilter` no longer re-derives a verdict from text for these tools.
