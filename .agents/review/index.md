...
required before integration. Current transport reaches the established model
but denies the required Bash guard command; affected branches remain
unintegrated until a valid transcript is available.

| ID | Severity | Impact (one line) | Status | Branch | Reviewer |
|----|----------|-------------------|--------|--------|----------|
| ci-rollback-1 | LOW | Windows CI hides the caught installer transaction exception | `[~]` | `fix/ci-rollback-diagnostic` | pending Opus guard-capable transport |
