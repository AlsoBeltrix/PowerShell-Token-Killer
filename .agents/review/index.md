...
**Closed — ci-slow-seal-2:** LOW — the slow output-seal test required an asynchronous timeout path to return strictly before its own two-second delay. Test-only commit `5180d0b` keeps the two-second seal limit behind an independent three-second elapsed bound and five-second caller budget. Mutation failed at `3.1491933s`; restored focused and 1,221/1,221 full server tests passed. Exact-head run `30748054339` passed all six jobs plus macOS-only rerun attempts 2 and 3 of that run (jobs `91497781569`, `91498323805`). Production remained unchanged. Reviewer: `claude/@gcp-vertexai-us-global-integration/anthropic.claude-opus-5/max/frontier`. Detail: `.agents/review/findings/ci-slow-seal-2.md`; plan: `.agents/plans/ci-slow-seal-elapsed-headroom.md`.
...
**Open — opr-22:** LOW — named-session startup starts its cancellation timer before first-use factory creation but passes a later wall-clock deadline, so a real startup timeout can be classified as `worker_start_canceled`. Status: accepted; plan required. Reviewer: `claude/@gcp-vertexai-us-global-integration/anthropic.claude-opus-5/max/frontier`. Detail: `.agents/review/findings/opr-22.md`.
