...
- **Slice 1 landed — post-success command advice deleted (2026-08-03):** the `PostSuccessGuidance` record, `TryCreateMixedDataflowGuidance`, `AllowsAdvisoryFileWriteGuidance`, the `allowFileSystemGuidance` parameter, and the warning-emission branch in `WithDispatchRouting` are gone. Mixed native/PowerShell pipelines execute exactly as before and return no rewritten-command suggestion. Seven advisory tests collapse into one regression, `Mixed_native_file_capture_executes_once_and_suggests_nothing`, proved by reintroducing an advisory warning (test fails) and restoring (passes). Closes `opr-58` (HIGH) as removed behavior — it could suggest redirecting a buffering producer into its own input, truncating the target to zero.

- **Slice 0 landed — the agent session no longer inherits user modules (2026-08-03):** `RunspaceHost.cs` now sets `PSModuleAutoloadingPreference = None` in the initial session state, keeping `PSModulePath` so AD/Exchange stay discoverable; explicit `Import-Module` is unaffected and stays warm. Three guards in `RunspaceHostTests.cs`; mutation proof confirmed guards 1 and 3 fail without the fix and guard 2 passes either way by design. **Scope correction:** an autoloaded module function does *not* override a built-in alias (alias outranks function), so the earlier claim that autoload rebinds `ls` was wrong — that observation came from a session where the user module was already imported by other means. The real defect is unrequested user code entering the agent's session and sessions diverging by invocation order, not alias hijacking.
