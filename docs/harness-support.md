...
| codex | `codex mcp add ptk -- <exe>` → `~/.codex/config.toml`. LIVE (tools listed). Installer leg shipped 2026-07-09 (`ptk_init.ps1 -Agent codex`, idempotent: an existing entry is left as-is). Install now repairs an orphaned `mcp_servers.ptk.tools.*` table before invoking the otherwise-bricked CLI (2026-09-04) | current TUI transcript records one outer `exec` call for both nested shell and MCP dispatch | none shipped: user hooks now have a documented trust flow, but the current TUI exposes no distinct selective shell tool contract; matching outer `exec` would also block PTK MCP calls | `~/.codex/AGENTS.md` — **VERIFIED 2026-07-09** by marker probe: a fresh `codex exec` session quoted the installed ptk-guidance block verbatim from its loaded context. Nudge installed on this box the same day |
