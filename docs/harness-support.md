...
| codex | `codex mcp add ptk -- <exe>` → `~/.codex/config.toml`. LIVE (tools listed). Installer leg shipped 2026-07-09 (`ptk_init.ps1 -Agent codex`, idempotent: existing entry left as-is). Install repairs orphaned `mcp_servers.ptk.tools.*` tables before invoking an otherwise-bricked CLI (2026-09-04) | canonical hook name `Bash`; MCP calls retain their own tool names | user-level `~/.codex/hooks.json` `PreToolUse` matcher `Bash`, merged without disturbing foreign hooks (shipped 2026-09-04 follow-up). Official Codex documentation confirms unified `exec_command`, including nested code-mode calls, is matched as `Bash` and supplies `tool_input.command`. Installer guard is green; next-session trust and live deny/reissue verification remain pending | `~/.codex/AGENTS.md` — **VERIFIED 2026-07-09** by marker probe: a fresh `codex exec` session quoted the installed ptk-guidance block verbatim from its loaded context. Nudge installed on this box the same day |
