...
| grok | `grok mcp add -s user ptk <exe>` → `~/.grok/config.toml`. LIVE (state call answered) | `ptk__ptk_state` | **No spillover**: grok did NOT honor the `~/.claude/settings.json` hook (a third-party ledger claimed it scans that file — wrong for this build). No grok hook shipped | **VERIFIED**: grok session-loads `~/.claude/CLAUDE.md` (marker probe quoted back) — the Claude user nudge covers grok |
| agy | `~/.gemini/config/mcp_config.json` `mcpServers` entry. LIVE (state call answered; headless auth worked this time) | unprefixed (`ptk_state`) | documented deny-with-guidance via customization-root/plugin `hooks.json` — live firing NOT yet demonstrated; plugin leg carries it (plan slice 4) | plugin `rules/*.md` (documented), or `~/.gemini/GEMINI.md` (does not exist yet) |
