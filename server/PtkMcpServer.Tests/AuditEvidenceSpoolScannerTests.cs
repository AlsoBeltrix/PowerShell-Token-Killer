...
public sealed class AuditEvidenceSpoolScannerTests
{
    ...
    {
        var ordinary = AuditCoreSchemaTestRecords.Create();
        AuditEvidenceSpoolScanner.ValidateExactEnvelopeShapeForTests(
            ordinary.Utf8Line[..^1]);

        var disposition = AuditCoreSchemaTestRecords.Create(
    ...
    public void Chain_codec_preserves_hash_linkage_across_supported_schema_versions()
    {
        ...
        var first = AuditSpoolRecordCodec.Parse(
            ...
            AuditCoreSchemaTestRecords.SupervisorBootId);
        var secondV2 = AuditCoreSchemaTestRecords.Create(
            sequence: 2,
    ...
    private static AuditOperatorDispositionFacts ResolvedDisposition() => new()
    {
        ...
        TargetSupervisorBootId = AuditCoreSchemaTestRecords.SupervisorBootId,
        ...
        TargetNextOffset = 20,
        TargetSequence = 7,
        TargetEventId = AuditCoreSchemaTestRecords.ParentEventId,
        FailureClass = "protocol",
        DetailCode = "otlp.bad_response",
        FirstFailureUtc = AuditCoreSchemaTestRecords.Occurred,
        TargetExportConfigurationIdentity = AuditCoreSchemaTestRecords.HashA,
        ProofKind = "verified_receipt",
        VerifiedReceiptDigest = AuditCoreSchemaTestRecords.HashA,
    };
}
