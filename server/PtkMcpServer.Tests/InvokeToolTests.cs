...
public sealed class InvokeToolTests : IDisposable
{
    ...
    public async Task Timeout_seals_the_emitted_prefix_as_an_incomplete_surviving_artifact()
    {
        ...
        try
        {
            ...
            var response = await (new SessionRuntime(host, _rawUsage)).InvokeAsync($"[IO.File]::AppendAllText('{escapedSentinel}', 'once'); " +
    ...
    public async Task Authorization_observes_exact_rtk_preparation_before_exit_reset_and_execution()
    {
        ...
        try
        {
            ...
            Assert.False(string.IsNullOrWhiteSpace(observed.ExecutionScript));
    ...
    /// <summary>
    /// Slice 1 regression: a successful mixed native/PowerShell pipeline
    /// executes exactly once and returns no rewritten-command suggestion.
    /// PostSuccessGuidance is deleted (it could suggest redirecting a
    /// buffering producer into its own input, truncating the target to zero);
    /// this pins that no advisory routing warning comes back.
    /// </summary>
    [Fact]
    public async Task Mixed_native_file_capture_executes_once_and_suggests_nothing()
    {
        var directory = Directory.CreateTempSubdirectory("ptk-mixed-no-guidance-");
        ...
        try
        {
            ...
            // Execution is unchanged: the original pipeline ran, once.
            Assert.True(result.Success, string.Join(Environment.NewLine, result.Errors));
            ...
            // No advisory rewrite comes back.
            Assert.DoesNotContain(result.Warnings, warning =>
                warning.Contains("[ptk:routing]", StringComparison.Ordinal));
            Assert.DoesNotContain(result.Warnings, warning =>
                warning.Contains("prefer", StringComparison.OrdinalIgnoreCase));
        }
        finally
        {
    ...
    public async Task Timeout_is_reported_with_the_state_loss_warning()
    {
        ...
        var text = await (new SessionRuntime(host, _rawUsage)).InvokeAsync("Start-Sleep -Seconds 60", CancellationToken.None);
    ...
    [Fact]
    public async Task Timeout_error_warns_against_automatic_resubmission()
    {
        using var host = new RunspaceHost(callTimeout: TimeSpan.FromSeconds(2));

        var text = await (new SessionRuntime(host, _rawUsage)).InvokeAsync("Start-Sleep -Seconds 60", CancellationToken.None);

        Assert.Contains("do not resubmit", text, StringComparison.OrdinalIgnoreCase);
