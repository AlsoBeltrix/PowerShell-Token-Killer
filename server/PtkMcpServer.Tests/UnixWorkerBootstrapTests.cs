...
public sealed class UnixWorkerBootstrapTests
{
    ...
    public void Bootstrap_rejects_wrong_direction_and_closes_both_duplicates()
    {
        ...
        Assert.Equal([13, 14], native.ClosedDuplicates.Order());
        Assert.Empty(native.OpenDescriptors);
    }

    [Theory]
    [InlineData(null, null, "handle_missing")]
    [InlineData("03", "4", "handle_invalid")]
    [InlineData("3", "04", "handle_invalid")]
    [InlineData("4", "3", "handle_invalid")]
    public void Bootstrap_rejects_nonbroker_descriptors_before_native_access(
        string? request,
        string? events,
        string detailCode)
    {
        ...
        var exception = Assert.Throws<WorkerBootstrapException>(() =>
            UnixWorkerBootstrap.Open(
                new WorkerBootstrapValues(request, events),
                native,
                isUnix: () => true));
