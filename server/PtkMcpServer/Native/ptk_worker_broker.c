...
#define PTK_MAGIC_3 UINT8_C(0x57)
#define PTK_PROTOCOL_VERSION UINT8_C(1)
...
static enum receive_result receive_command(enum command_kind *kind)
{
    ...
    enum receive_result result =
        read_control_bytes(header, sizeof(header));
    if (result != RECEIVE_OK) {
        return result;
    }
    if (header[0] != PTK_MAGIC_0 ||
        header[1] != PTK_MAGIC_1 ||
        header[2] != PTK_MAGIC_2 ||
        header[3] != PTK_MAGIC_3 ||
    ...
    struct process_identity *identity)
{
    ...
    return identity->high != UINT64_C(0) ||
        identity->low != UINT64_C(0);
}
#else
#error "PtkWorkerBroker supports only Linux and macOS."
#endif
    ...
    char *const worker_arguments[])
{
    ...
    if (received != (ssize_t)sizeof(release) ||
        release != UINT8_C(1)) {
        _exit(71);
    }
    ...
    char *const worker_arguments[])
{
    ...
    if (received != RECEIVE_OK) {
        return fail_started_worker(
            STAGE_CHILD_SETUP,
            received == RECEIVE_LIVENESS_LOST
                ? ECANCELED
                : EPROTO,
            worker_pid,
            worker_identity,
    ...
    if (!identity_is_live(worker_pid, worker_identity) ||
        (setpgid(worker_pid, worker_pid) != 0 &&
            errno != EACCES)) {
        return fail_started_worker(
            STAGE_GROUP_ARM,
            errno == 0 ? ESRCH : errno,
...
{
    if (argc < 4 ||
        strcmp(argv[1], "--worker-broker-v1") != 0 ||
        argv[2][0] != '/' ||
        argv[3][0] != '/') {
        return 64;
    }
