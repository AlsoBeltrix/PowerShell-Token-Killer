...
/// <summary>
/// First-action Unix worker bootstrap. The native broker maps the request and
/// event pipes to descriptors 3 and 4. The worker immediately marks both
/// close-on-exec, duplicates them into managed ownership, closes the inherited
/// originals, validates their direction, and leaves no bootstrap descriptor
/// available to user descendants.
/// </summary>
internal static class UnixWorkerBootstrap
{
        ...
        Func<bool>? isUnix = null)
    {
        ...
        if (!string.Equals(
                ...
                RequestDescriptor.ToString(
                    System.Globalization.CultureInfo.InvariantCulture),
                StringComparison.Ordinal) ||
            !string.Equals(
                values.EventHandle,
                EventDescriptor.ToString(
                    System.Globalization.CultureInfo.InvariantCulture),
                StringComparison.Ordinal))
        {
    ...
    private static void RequireAccess(
        ...
        FileAccess expected)
    {
        var actual = Invoke("handle_invalid", () => native.GetAccess(descriptor));
        if (actual != expected)
