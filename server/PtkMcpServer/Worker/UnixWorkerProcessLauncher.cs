...
/// broker. The broker forks a gated worker in the supervisor group; managed
/// code records the exact broker/worker identities before allowing the group
...
internal sealed class UnixWorkerProcessLauncher : IWorkerProcessLauncher
{
    ...
    private static readonly TimeSpan BrokerContainmentTimeout =
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        catch (Exception exception) when (!IsFatal(exception))
        {
            if (authority is not null)
            {
                ...
                var containmentEmpty = authority.ContainmentEmpty;
                authority.Dispose();
                if (containment.Outcome ==
                    WorkerContainmentOutcome.DescendantsUnknown)
                {
                    throw new WorkerProcessException(
                        "unix_worker_launch_containment_unconfirmed",
                        exception,
                        containmentEmpty);
                }
            }
    ...
    private sealed class ContainedUnixWorker : IWorkerContainedProcess
    {
        ...
        private int _lateBrokerObservationStarted;
        ...
        public int ContainmentProcessId => _brokerProcessId;
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (!SameBroker(hello, gated) ||
                ...
                _native.GetProcessGroup(gated.WorkerProcessId) !=
                    supervisorGroup)
            {
                throw InvalidBrokerEvent();
            }
        ...
        private async Task<WorkerContainmentResult> ContainCoreAsync()
        {
            ...
            DisposeIgnoringFailure(_pipes.Commands);
            DisposeIgnoringFailure(_pipes.Liveness);

            int? exitCode;
            try
            {
                exitCode = await _brokerExit
            ...
            var brokerConfirmed =
                exitCode is { } code && IsConfirmedBrokerExit(code);
            if (!_registered || _identity is not { } identity)
            {
                if (brokerConfirmed)
                {
        ...
        private void ObserveBrokerExitLater()
        {
            if (Interlocked.CompareExchange(
                    ref _lateBrokerObservationStarted,
                    1,
                    0) == 0)
            {
                _ = CompleteBrokerExitLaterAsync();
            }
        }

        private async Task CompleteBrokerExitLaterAsync()
        {
            int exitCode;
            try
            {
                exitCode = await _brokerExit.ConfigureAwait(false);
            }
            catch (Exception exception) when (!IsFatal(exception))
            {
                return;
            ...
            if (!IsConfirmedBrokerExit(exitCode))
                return;
            if (!_registered || _identity is not { } identity)
            {
                _containmentEmpty.TrySetResult();
        ...
        private async Task ForwardContainmentEmptyAsync(Task registryEmpty)
        {
            try
            {
                await registryEmpty.ConfigureAwait(false);
                _containmentEmpty.TrySetResult();
            }
            catch (Exception exception) when (!IsFatal(exception))
            {
        ...
        private static bool IsConfirmedBrokerExit(int exitCode) =>
            exitCode is 0 or 64 or 70 or 71 or 72 or 73;
        ...
        public void Dispose()
        {
            ...
            if (failures is { Count: > 1 })
            {
                throw new AggregateException(
                    "Unix worker pipe cleanup failed.",
                    failures);
            }
        }
...
internal static class UnixWorkerBrokerProtocol
{
        ...
        ushort payloadLength)
    {
        ...
        {
            throw new ArgumentException(
                "Broker header destination is too short.",
                nameof(destination));
        }
...
{
    private static readonly TimeSpan ProcessTableCacheLifetime =
    ...
    public bool ProcessGroupExists(int processGroup)
    {
        if (processGroup <= 0)
            return false;
        var result = KillProcess(-processGroup, 0);
        return result == 0 || Marshal.GetLastPInvokeError() == PermissionDenied;
    }

    public List<PtkMcpServer.ProcessTableRow>? TryTakeProcessTable() =>
        PtkMcpServer.ProcessTableSnapshot.TryTakeShared(
